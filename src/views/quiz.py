...
class QuizView:
    ...
    def quiz_answer(self, message, match):
        ...
        if quiz:
            ...
            if ans == quiz["correct_alternative"]:
                return TextMessageProtocolEntity("Certa resposta!", to=message.getFrom())
            else:
                msg_wrong = "Erroooou.. A certa era: %s" % quiz["correct"]
    ...
    def _get_quiz(self):
        ...
        return {
            "question": "Quanto é %s?" % " + ".join(str(v) for v in values),
